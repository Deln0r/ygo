...
- **Federated transport over Matrix.** An opt-in [Matrix transport](integration/matrix) (a separate module, so mautrix stays out of a build that only wants the CRDT) turns any Matrix room into the sync channel: peers publish updates as `dev.ygo.update` room events and merge what they read back, with no ygo server involved. A room is an append-only log with no ordering or exactly-once guarantee, which is exactly what Yjs updates tolerate. Room content is treated as untrusted — validated on both sides, decoded one event at a time, encrypted rooms refused rather than silently written in the clear — and the whole thing is exercised in CI against a real Dendrite on every push. Its limits are documented rather than papered over.
