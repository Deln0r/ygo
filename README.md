...
Shipped since v1.0 (June 2026): a full Hocuspocus-compatible sync server (connection lifecycle hooks, read-only viewers, resource caps, a stats snapshot), an offline-first Go client, WAL-backed SQLite persistence, continuous fuzzing, and the yjs update-level utilities. See the [releases](https://github.com/Deln0r/ygo/releases) and [examples/](examples).
